...
class TestResult:
    ...
    ai_reasoning_quality: Optional[int] = None  # 1-5 scale
...
class ComprehensiveFunctionCallingTester:
    """
    Comprehensive tester for function calling capabilities
    Tests both technical functionality and real AI behavior
    """
    ...
    def analyze_ai_response(self, response: Dict[str, Any], test_def: FunctionCallTest) -> TestResult:
        ...
        try:
            ...
            if "data" in response and "response" in response["data"]:
                ...
                function_calls_executed = parsed_response.get("function_calls_executed", False)
                
                # Analyze reasoning quality
                response_content = parsed_response.get("text", "")
                reasoning_score = self.score_ai_reasoning(response_content, test_def.reasoning_keywords)
                
                # Check if expected functions were likely called
                expected_function_detected = any(keyword.lower() in response_content.lower() 
    ...
    def score_ai_reasoning(self, response_text: str, keywords: List[str]) -> int:
        ...
        quality_indicators = [
            ...
            "?" not in response_text or response_text.count("?") <= 2,  # Not too many questions
            any(word in response_lower for word in ["sprawdzam", "wykonuję", "znajduję", "dodaję"]),  # Action words
        ]
    ...
    def generate_coverage_report(self) -> Dict[str, Any]:
        ...
        # Calculate averages
        avg_reasoning_quality = sum(r.ai_reasoning_quality or 0 for r in self.results) / total_tests if total_tests > 0 else 0
        ...
        function_calling_coverage = (function_call_tests / total_tests * 100) if total_tests > 0 else 0
    ...
    def print_coverage_report(self, report: Dict[str, Any]):
        ...
        print(f"   Function Calling Coverage: {summary['function_calling_coverage']:.1f}%")
        ...
        for category, stats in report["categories"].items():
            ...
            fc_rate = (stats["function_calls"] / stats["total"] * 100) if stats["total"] > 0 else 0
            print(f"   {category.upper()}: {stats['success']}/{stats['total']} ({success_rate:.1f}%) - FC: {fc_rate:.1f}%")
    ...
    async def save_report(self, report: Dict[str, Any], filename: str = "comprehensive_function_calling_report.json"):
