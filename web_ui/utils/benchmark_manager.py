...
class AssistantBenchmark:
    ...
    def benchmark_database_operations(self) -> dict:
        ...
        try:
            ...
            insert_id = (
                cursor.lastrowid
            )  # Fixed: use cursor.lastrowid instead of conn.lastrowid
            ...
            start_time = time.time()
            cursor = conn.execute(
                "SELECT * FROM conversations WHERE id = ?", (insert_id,)
            )
    ...
    def benchmark_tts_generation(self) -> dict:
        ...
        try:
            ...
            chars_per_second = (
                len(test_text) / generation_time if generation_time > 0 else 0
            )
            ...
            if hasattr(assistant, "ai_module") and assistant.ai_module:
                ...
                try:
                    ...
                    return {
                        ...
                        "chars_per_second": (
                            round(response_length / response_time, 1)
                            if response_time > 0
                            else 0
                        ),
                        "response_preview": (
