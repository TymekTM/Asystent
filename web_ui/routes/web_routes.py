...
def setup_web_routes(app):
    ...
    def index():
        ...
        if hasattr(get_assistant_instance(), 'wake_word_detector'):
            try:
                wake_word_active = get_assistant_instance().wake_word_detector.is_running()
    ...
    def dev_add_user():
        ...
        add_user(username=username, password=password_hash, role=role, 
                display_name=display_name, ai_persona=ai_persona, 
    ...
    def dev_delete_user():
        ...
        username = data.get('username')
        if username == 'dev':
            return jsonify({'success': False, 'error': 'Cannot delete the default dev user.'}), 400
