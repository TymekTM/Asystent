...
from server.server_main import ServerApp, app as server_app
from client.client_main import ClientApp
from conftest import create_test_message
...
class TestWebSocketCommunication:
    ...
    async def test_websocket_connection_flow(self, server_app, client_app):
        ...
        assert "Hello client!" in response["response"]
    ...
    @pytest.mark.asyncio
    async def test_plugin_communication_flow(self, server_app, client_app, mock_plugin):
        """Test komunikacji pluginów między klientem a serwerem."""
        user_id = "plugin_test_user"
        
        with patch('server.server_main.plugin_manager') as mock_plugin_manager:
            # Setup plugin manager
            mock_plugin_manager.get_all_plugins.return_value = {"test_plugin": mock_plugin}
            mock_plugin_manager.get_user_plugins.return_value = {}
            mock_plugin_manager.enable_plugin_for_user.return_value = True
            ...
            enable_response = await server_app.process_user_request(user_id, enable_request)
            ...
            with patch.object(server_app, 'load_plugin_module', return_value=mock_plugin):
                call_request = {
                    "type": "function_call",
                    "plugin": "test_plugin",
                ...
                call_response = await server_app.process_user_request(user_id, call_request)
    ...
    async def test_daily_briefing_flow(self, server_app, client_app):
        ...
        # Mock daily briefing module
        mock_briefing = {
            "weather": "Sunny, 22°C",
            "calendar": "3 meetings today",
            "news": "Tech update"
        }
        server_app.daily_briefing.generate_daily_briefing.return_value = mock_briefing
        
        # 1. Klient prosi o briefing startowy
        briefing_request = {"type": "startup_briefing"}
        briefing_response = await server_app.process_user_request(user_id, briefing_request)
        ...
        with patch.object(client_app, 'format_briefing_text', return_value="Daily briefing text"):
            await client_app.handle_startup_briefing(mock_briefing)
        
        client_app.tts.speak.assert_called_once_with("Daily briefing text")
    ...
    async def test_concurrent_users(self, server_app):
        ...
        for user in users:
            mock_websockets[user].send_text.assert_called_once_with(json.dumps(broadcast_message))
...
class TestAudioPipelineIntegration:
    ...
    @pytest.mark.asyncio
    async def test_audio_device_failure_recovery(self, client_app):
        """Test odzyskiwania po awarii urządzenia audio."""
        # Symuluj błąd urządzenia audio
        with patch('client.client_main.create_wakeword_detector', side_effect=Exception("Audio device error")):
            with patch('client.client_main.create_whisper_asr', side_effect=Exception("ASR error")):
                with patch('client.client_main.TTSModule', side_effect=Exception("TTS error")):
...
class TestOverlayIntegration:
    ...
    async def test_overlay_status_updates(self, client_app):
        ...
        # Sprawdź czy overlay otrzymało aktualizacje
    ...
    def test_overlay_http_endpoints(self, client_app):
        ...
        from client.client_main import StatusHTTPHandler
        import queue
        ...
        assert response_data["status"] == "Test Status"
        assert response_data["wake_word_detected"] is False
        assert response_data["tts_playing"] is False
...
class TestPerformanceIntegration:
    ...
    async def test_high_frequency_messages(self, server_app, client_app):
        ...
        assert avg_time_per_message < 1.0, f"Average processing time: {avg_time_per_message:.3f}s"
...
class TestSecurityIntegration:
    ...
    async def test_input_sanitization(self, server_app):
        ...
        malicious_inputs = [
            {"type": "ai_query", "query": "<script>alert('xss')</script>", "context": {}},
            ...
            {"type": "ai_query", "query": "A" * 10000, "context": {}}  # Bardzo długi input
        ]
        
        for malicious_input in malicious_inputs:
            try:
                response = await server_app.process_user_request(user_id, malicious_input)
