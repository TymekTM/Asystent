...
class TestServerPerformance:
    ...
    async def test_concurrent_connections(self, server_app):
        ...
        mock_websockets = []
        for _ in range(num_connections):  # i not used
            mock_ws = AsyncMock()
        ...
        assert (
            connect_time < 5.0
        ), f"Connection time: {connect_time:.2f}s for {num_connections} users"
        ...
        assert (
            broadcast_time < 2.0
        ), f"Broadcast time: {broadcast_time:.2f}s for {num_connections} users"
    ...
    async def test_concurrent_request_processing(self, server_app):
        ...
        print(
            f"Concurrent processing: {concurrent_time:.2f}s for {total_requests} requests"
        )
    ...
    async def test_memory_usage_under_load(self, server_app):
        ...
        for iteration in range(num_iterations):
            ...
            if memory_increase > 200:  # 200MB limit
                pytest.fail(
                    f"Memory usage increased by {memory_increase:.1f}MB after {iteration} iterations"
                )
...
class TestClientPerformance:
    ...
    async def test_overlay_update_performance(self, client_app):
        ...
        with patch.object(
            client_app, "send_overlay_update"
        ) as _:  # mock_update not used
            start_time = time.time()
        ...
        ), f"Overlay update rate too low: {updates_per_second:.1f} updates/s"
    ...
    async def test_command_queue_performance(self, client_app):
        ...
        for i in range(num_commands):
            command = {"type": "status_update", "status": f"Command {i}"}
        ...
        ), f"Command processing too slow: {commands_per_second:.1f} cmd/s"
        assert (
            processed == num_commands
        ), f"Not all commands processed: {processed}/{num_commands}"
...
class TestIntegratedPerformance:
    ...
    async def test_full_conversation_performance(self, server_app, client_app):
        ...
        print("Conversation performance:")
    ...
    async def test_system_stability_under_load(self, server_app, client_app):
        ...
        async def client_load_generator(client_id, duration, rate):
            ...
            while time.time() < end_time:
                ...
                try:
                    await server_app.process_user_request(
                        f"load_client_{client_id}", request
                    )
                    request_count += 1
        ...
        for client_id in range(num_clients):
            task = client_load_generator(
                client_id, duration_seconds, requests_per_client_per_second
            )
        ...
        # Analiza wyników
        total_requests = sum(
            count for count in request_counts if isinstance(count, int)
        )
        expected_requests = (
            num_clients * duration_seconds * requests_per_client_per_second
        )
        ...
        assert (
            success_rate > 0.8
        ), f"System stability too low: {success_rate * 100:.1f}% success rate"
...
class TestResourceUsage:
    ...
    def test_memory_leak_detection(self):
        ...
        assert (
            memory_retained < 10
        ), f"Possible memory leak: {memory_retained:.1f}MB retained"
