...
class TestWakewordDetection:
    ...
    @pytest.mark.asyncio
    async def test_wakeword_integration_with_client(self, client_app, mock_audio_components):
...
class TestAudioRecorder:
    ...
    def test_recording_duration(self, mock_audio_components):
        ...
        for duration in durations:
            ...
            expected_size = int(duration * 16000 * 2)  # 16kHz, 16-bit
...
class TestTTSModule:
    ...
    async def test_tts_speak_long_text(self, mock_audio_components):
        ...
        # Long text
        long_text = "This is a very long text message that should be handled properly by the TTS system. " * 10
...
class TestAudioDevices:
    ...
    def test_list_audio_devices(self):
        ...
        with patch('client.client_main.list_audio_devices') as mock_list:
            mock_devices = [
                {"name": "Built-in Microphone", "index": 0, "channels": 1, "type": "input"},
                {"name": "Built-in Speakers", "index": 1, "channels": 2, "type": "output"},
            ...
            from client.client_main import list_audio_devices
    ...
    def test_audio_device_selection(self, client_app):
        ...
        with patch.object(client_app, 'list_available_audio_devices', return_value=mock_devices):
    ...
    async def test_audio_device_error_handling(self, client_app):
        ...
        with patch.object(client_app, 'list_available_audio_devices', side_effect=Exception("No audio devices")):
...
class TestAudioPipeline:
    ...
    @pytest.mark.asyncio
    async def test_audio_pipeline_error_recovery(self, client_app, mock_audio_components):
        """Test odzyskiwania po błędach w pipeline."""
        client_app.audio_recorder = mock_audio_components["audio_recorder"]
        client_app.whisper_asr = mock_audio_components["whisper_asr"]
        
        # Symuluj błąd nagrywania
        client_app.audio_recorder.get_audio_data.side_effect = Exception("Recording failed")
        
        with pytest.raises(Exception, match="Recording failed"):
            await client_app.record_and_transcribe()
        
        # Reset błędu i test normalnego działania
        client_app.audio_recorder.get_audio_data.side_effect = None
