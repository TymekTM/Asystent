            ...
            try:
                if hasattr(model, '_models'):
                    ...
                    if model_count >= 4:
                        ...
                        logger.warning(f"⚠️ Only {model_count} models loaded, expected 4")
                        return True  # Still successful, just fewer models
                else:
                    logger.info("✅ OpenWakeWord model loaded (model count check not available)")
                    return True
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(f"http://localhost:{port}/api/status") as response:
                        ...
                        if response.status == 200:
                            logger.info(f"✅ HTTP server on port {port} responds in {response_time:.1f}ms")
                            if response_time < 100:  # Less than 100ms is good
                                ...
                                logger.warning(f"⚠️ Overlay responsiveness: SLOW ({response_time:.1f}ms)")
                                return True
                        else:
                            logger.warning(f"⚠️ HTTP server returned status {response.status}")
                            continue
            except Exception as e:
                logger.debug(f"Port {port} not available: {e}")
                continue
        
        logger.warning("⚠️ No HTTP server found running - overlay may not be responsive")
        return False
...
async def test_click_through():
    ...
    try:
        ...
        logger.info("  - Window position: set to HWND_BOTTOM for enhanced click-through")
