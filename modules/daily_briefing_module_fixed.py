...
class DailyBriefingModule:
    ...
    async def get_weather_data(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            from modules.weather_module import handle_weather_request
            
            # Get weather information
            weather_result = await handle_weather_request(
                f"pogoda {self.location}",
                {"location": self.location}
            )
            
            if weather_result and weather_result.get('success'):
                return {
                    'current_text': weather_result.get('message', ''),
                    'location': self.location
                }
        except Exception as e:
