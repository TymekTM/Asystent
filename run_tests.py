...
def check_test_environment():
    ...
    required_modules = [
        "asyncio", "json", "unittest.mock", "pathlib"
    ]
...
def main():
    ...
    parser.add_argument(
        "--type", "-t",
        choices=["all", "unit", "integration", "performance", "audio", "server", "client", "fast", "slow"],
        default="all",
    ...
    parser.add_argument(
        "--coverage", "-c",
        action="store_true",
        help="Generuj raport pokrycia kodu"
    )
    
    parser.add_argument(
        "--parallel", "-p",
        action="store_true",
        help="Uruchom testy równolegle"
    )
    
    parser.add_argument(
        "--markers", "-m",
        help="Dodatkowe markery pytest"
    )
    
    parser.add_argument(
        "--file", "-f",
        help="Konkretny plik testowy do uruchomienia"
    )
