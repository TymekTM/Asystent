...
class DependencyManager:
    ...
    def __init__(self):
        ...
        self.log_file = self.deps_dir / "installation.log"
        
        # Konfiguruj logging dla instalacji
        self._setup_logging()
        
    def _setup_logging(self):
        """Konfiguruje logging dla procesu instalacji."""
        if not self.deps_dir.exists():
            self.deps_dir.mkdir(exist_ok=True)
            
        # Utwórz handler do pliku logów
        ...
        # Dodaj handler do loggera jeśli jeszcze nie ma
    ...
    def check_missing_packages(self) -> List[str]:
        """Sprawdza które pakiety brakują - tylko te które nie są wbudowane w EXE."""        # Minimalna lista - tylko to co jest absolutnie potrzebne i nie może być wbudowane
        required_packages = [
            ...
            # Specjalistyczne
            "deepseek",
            # "whisper",  # Usunięte - nie jest potrzebne
        ]
    ...
    def _download_openwakeword_models(self):
        """Pobiera standardowe modele ONNX dla openwakeword."""
        openwakeword_dir = self.packages_dir / "openwakeword"
        if not openwakeword_dir.exists():
            return False
            
        resources_dir = openwakeword_dir / "resources"
        models_dir = resources_dir / "models"
        
        # Utwórz foldery jeśli nie istnieją
        models_dir.mkdir(parents=True, exist_ok=True)
        
        # Lista wymaganych modeli standardowych
        required_models = {
            "melspectrogram.onnx": "https://github.com/dscripka/openWakeWord/raw/main/openwakeword/resources/models/melspectrogram.onnx",
            "alexa_v0.1.onnx": "https://github.com/dscripka/openWakeWord/raw/main/openwakeword/resources/models/alexa_v0.1.onnx",
            "hey_jarvis_v0.1.onnx": "https://github.com/dscripka/openWakeWord/raw/main/openwakeword/resources/models/hey_jarvis_v0.1.onnx"
        }
        
        logger.info(f"📥 Pobieranie modeli ONNX dla openwakeword...")
        print(f"   📥 Pobieranie modeli ONNX dla openwakeword...")
        
        success_count = 0
        for model_name, url in required_models.items():
            model_path = models_dir / model_name
            if model_path.exists():
                logger.info(f"   ✅ {model_name} (już istnieje)")
                success_count += 1
                continue
            ...
            try:
                ...
                logger.info(f"   ✅ {model_name} - pobrano")
                print(f"     ✅ {model_name}")
                success_count += 1
        ...
        logger.info(f"📊 Pobrano modeli: {success_count}/{len(required_models)}")
        print(f"   📊 Pobrano modeli: {success_count}/{len(required_models)}")
...
def ensure_dependencies():
    ...
    if not manager.is_initialized():
        ...
        if missing:
            print(f"\n❌ Brakuje {len(missing)} pakietów: {', '.join(missing)}")
            print("📥 Automatyczne pobieranie brakujących pakietów...")
            if manager.install_missing_packages(missing):
                print("✅ Instalacja zakończona pomyślnie")
            else:
                print("⚠️ Niektóre pakiety mogły nie zostać zainstalowane")
        else:
            print(f"\n✅ Wszystkie pakiety dostępne!")
          # Utwórz plik blokady z listą zainstalowanych pakietów
        manager._create_lock_file(missing if missing else [])
    else:
        print("✅ Zależności już sprawdzone i zainstalowane.")
