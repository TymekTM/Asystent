...
class DependencyManager:
    ...
    async def check_installation_needed(self) -> bool:
        ...
        try:
            ...
            essential_deps = {name for name, dep in self.HEAVY_DEPENDENCIES.items() if dep.essential}
    ...
    async def install_dependencies(self, force: bool = False) -> bool:
        ...
        essential_deps = {name: dep for name, dep in self.HEAVY_DEPENDENCIES.items() if dep.essential}
        ...
        for name, dependency in essential_deps.items():
            try:
                logger.info(f"Installing {dependency.name} ({dependency.description})...")
    ...
    async def _create_lock_file(self) -> None:
        ...
        async with aiofiles.open(self.lock_file, 'w') as f:
            await f.write(f"Dependencies installed successfully\nTimestamp: {asyncio.get_event_loop().time()}\n")
