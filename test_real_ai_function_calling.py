#!/usr/bin/env python3
"""
TEST RZECZYWISTEGO FUNCTION CALLING Z OpenAI API GPT-4.1-nano
Łączy się z rzeczywistym serwerem Gaja i wykonuje prawdziwe zapytania AI.
"""
...
class RealAIFunctionTest:
    ...
    async def check_api_key(self) -> bool:
        ...
        if api_key.startswith('sk-') and len(api_key) > 20:
            print(f"✅ Klucz API OpenAI jest skonfigurowany: {api_key[:10]}...{api_key[-4:]}")
            return True
        else:
            print(f"❌ Nieprawidłowy format klucza API: {api_key[:20]}...")
            return False
    
    async def test_rest_api_query(self, query: str, expected_functions: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(f"{self.server_url}/api/v1/ai/query", json=payload) as response:
                    if response.status == 200:
                        data = await response.json()
                        return {
                            "success": True,
    ...
    async def test_websocket_query(self, query: str, expected_functions: List[str] = None) -> Dict[str, Any]:
    ...
    async def run_test_scenario(self, scenario: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(f"🎯 Oczekiwane funkcje: {', '.join(scenario.get('expected_functions', []))}")
        ...
        rest_result = await self.test_rest_api_query(
            scenario['query'], 
            scenario.get('expected_functions')
        )
        results["tests"].append(rest_result)
        
        if rest_result["success"]:
            print(f"✅ REST API: Sukces")
            print(f"📄 Odpowiedź: {json.dumps(rest_result['response'], ensure_ascii=False, indent=2)[:200]}...")
        else:
        ...
        if ws_result["success"]:
            print(f"✅ WebSocket: Sukces")
            print(f"📄 Odpowiedź: {json.dumps(ws_result['response'], ensure_ascii=False, indent=2)[:200]}...")
        else:
    ...
    async def run_comprehensive_test(self):
        ...
        test_scenarios = [
            ...
            },
            {
                ...
                "expected_functions": ["music_module_control_music", "core_module_get_current_time"]
            },
            {
                "name": "Kalendarz i przypomnienia",
                "query": "Dodaj wydarzenie na jutro o 15:00 i ustaw przypomnienie",
                "expected_functions": ["core_module_add_event", "core_module_set_reminder"]
            },
            {
