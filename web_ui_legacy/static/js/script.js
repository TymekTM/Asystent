...
document.addEventListener("DOMContentLoaded", function () {
    ...
    addMemoryForm.addEventListener("submit", function (event) {
      ...
      fetch("/api/ltm/add", {
        ...
        .then(({ status, body }) => {
          if (status === 201) {
            ...
            fetchAndDisplayMemories(
              searchMemoryInput ? searchMemoryInput.value : "",
            );
          } else {
            addMemoryStatus.textContent = `Błąd: ${
              body.error || "Nie udało się dodać wspomnienia."
            }`;
        ...
        .catch((error) => {
          ...
          addMemoryStatus.textContent =
            "Wystąpił błąd sieciowy podczas dodawania.";
          addMemoryStatus.className = "mt-2 text-danger";
        })
        .finally(() => {
  ...
  if (clearSearchButton && searchMemoryInput) {
    // Added listener for clear button
      ...
      if (memories.length > 0) {
        ...
        memories.forEach((memory) => {
          ...
          const timestamp = memory.timestamp
            ? memory.timestamp.split(".")[0]
            : "N/A"; // Format timestamp

          li.innerHTML = `
                        <div class="ms-2 me-auto">
                            <div class="fw-bold">ID: ${memory.id} | User: ${
                              memory.user || "N/A"
                            }</div>
                            <small class="text-muted">${timestamp}</small>
                            <p class="mb-1">${escapeHtml(memory.content)}</p>
                        </div>
                        <button class="btn btn-danger btn-sm delete-memory-button" data-id="${
                          memory.id
                        }">Usuń</button>
                    `;
          ul.appendChild(li);
        ...
        memoryList.innerHTML =
          "<p>Brak zapisanych wspomnień" +
          (query ? " pasujących do wyszukiwania." : ".") +
          "</p>";
      }
    })
    .catch((error) => {
      ...
      memoryList.innerHTML =
        '<p class="text-danger">Wystąpił błąd podczas pobierania wspomnień.</p>';
    });
}
          ...
          .then(({ status, body }) => {
            if (status === 200) {
              ...
              statusDiv.textContent = `Błąd: ${
                body.error || "Nie udało się usunąć wspomnienia."
              }`;
