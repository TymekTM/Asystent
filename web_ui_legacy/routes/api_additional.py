...
from core.config import load_main_config, logger
...
def setup_additional_api_routes(app):
    ...
    def chat_send_api():
        ...
        ai_content = (
            ...
            if ai_response and ai_response.get("message", {}).get("content")
            else None
        )
        reply = ""
        ...
        if ai_content:
            try:
                ...
                if isinstance(parsed, dict) and (
                    parsed.get("command") or parsed.get("tool")
                ):
                    # Tool call detected
                    ...
                    for mod_name in tool_modules:
                        try:
                            ...
                            if hasattr(mod, "register"):
                                ...
                                if reg.get(
                                    "command"
                                ) == ai_command or ai_command in reg.get("aliases", []):
                    ...
                    if handler:
                        ...
                        try:
                            ...
                            tool_result = (
                                f"Błąd podczas wywołania funkcji '{ai_command}': {e}"
                            )
                        ...
                        if isinstance(tool_result, tuple):
                            ...
                            reply = (
                                ai_text or f"(Brak odpowiedzi narzędzia {ai_command})"
                            )
                    else:
                        reply = (
                            ai_text
                            or f"(Nie znaleziono handlera dla komendy {ai_command})"
                        )
    ...
    def api_plugin_enable(name):
        ...
        plugins_file = os.path.join(
            os.path.dirname(__file__), "..", "..", "plugins_state.json"
        )
        try:
            state = {}
            if os.path.exists(plugins_file):
    ...
    def api_logs():
        ...
        log_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "user_data", "assistant.log"
        )
        ...
        try:
            if not os.path.exists(log_path):
                ...
                return jsonify(
                    {
                        "logs": [
                            "Log file not found. A new log file will be created when new events occur."
                        ],
                        "page": 1,
                        "total_pages": 1,
                        "total_lines": 1,
    ...
    def api_analytics():
        ...
        try:
            ...
            last_query_time = max(
                (entry.get("timestamp", 0) for entry in history), default=None
            )
    ...
    def api_playground_plugins():
        ...
        try:
                    ...
                    for sub_name, sub_info in sub_commands.items():
                        if isinstance(sub_info, dict):
                            sub_cmd_obj = {
                                ...
                                "function_name": (
                                    getattr(
                                        sub_info.get("function"), "__name__", "unknown"
                                    )
                            ...
                            if "parameters" in sub_info and isinstance(
                                sub_info["parameters"], dict
                            ):
                                parameters_array = []
                                for param_name, param_info in sub_info[
                                    "parameters"
                                ].items():
                                    param_obj = {
                                        "name": param_name,
            ...
            logger.error(
                f"Error getting plugin info for playground: {e}", exc_info=True
            )
            ...
            try:
                    ...
                    if "function" in sub_info:
                        ...
                        for param_name in sig.parameters:
                            if param_name in execution_params:
                                filtered_params[param_name] = execution_params[
                                    param_name
                                ]
                ...
                if result is not None:
                    if isinstance(result, dict):
                        ...
                        error = (
                            "Handler returned coroutine but was not awaited properly"
                        )
                        result = None
                    else:
                        result = str(result)
            ...
            for sub_name, sub_info in sub_commands.items():
                if isinstance(sub_info, dict):
                    ...
                    detailed_info["sub_commands"][sub_name] = {
                        ...
                        "params_desc": sub_info.get("params_desc", ""),
                        "function_name": (
                            getattr(sub_info.get("function"), "__name__", "unknown")
                            if "function" in sub_info
                            else None
                        ),
    ...
    def api_playground_llm_test():
        ...
        try:
                ...
                if dry_run:
                    ...
                    try:
                        ...
                        if ai_response and "message" in ai_response:
                            result_data["ai_response"] = ai_response["message"][
                                "content"
                            ]
                            # Estimate tokens (rough calculation)
                            result_data["tokens_in"] = (
                                len(prompt_context.split()) * 1.3
                            )  # Rough estimate
                            result_data["tokens_out"] = (
                                len(result_data["ai_response"].split()) * 1.3
                            )
                        else:
                    ...
                    if plugin:
                        ...
                        if plugin in plugins:
                            ...
                            if "handler" in plugin_info:
                                ...
                                for param_name in sig.parameters:
                                    if param_name in filtered_params:
                                        final_params[param_name] = filtered_params[
                                            param_name
                                        ]
                            ...
                            result_data[
                                "error"
                            ] = f"Plugin '{plugin}' nie został znaleziony"
                    else:
                        ...
                        result_data["ai_response"] = response.get(
            ...
            return jsonify({"success": result_data["error"] is None, **result_data})
    ...
    def api_playground_direct_llm_test():
        ...
        try:
            ...
            model = data.get(
                "model", "auto"
            )  # Model selection might depend on provider
            prompt = data.get("prompt", "")
            ...
            try:
                ...
                from server.ai_module import (
                    chat_with_providers,  # Assuming this handles provider/model selection
                )
                ...
                if provider != "auto" and provider != current_config.get("AI_PROVIDER"):
                    original_provider_config = current_config.get(
                        "AI_PROVIDER", "openai"
                    )
                    current_config["AI_PROVIDER"] = provider
                ...
                if (
                    ai_response_data
                    and "message" in ai_response_data
                    and ai_response_data["message"].get("content")
                ):
                    ...
                    result_data["tokens_out"] = len(
                        result_data["ai_response"].split()
                    )  # Rough estimate
                    result_data["token_count"] = (
                        result_data["tokens_in"] + result_data["tokens_out"]
                    )
                    ...
                    result_data["estimated_cost"] = (
                        result_data["tokens_in"] / 1000 * cost_per_1k_tokens_in
            ...
            for provider_id, provider_name in providers.items():
                if provider_id == "auto":
                    ...
                    if api_keys.get(key_name):
                        ...
                        available_providers[
                            provider_id
                        ] = f"{provider_name} (brak klucza API)"
    ...
    def api_playground_plugin_test():
        ...
        try:
                ...
                if dry_run:
                    ...
                    try:
                        ...
                        if hasattr(plugin_module, function_name):
                            ...
                            result_data[
                                "error"
                            ] = f"Function '{function_name}' not found in plugin '{plugin}'"
            ...
            end_time = datetime.utcnow()
            result_data["execution_time"] = int(
                (end_time - start_time).total_seconds() * 1000
            )
    ...
    def api_playground_test_history_enhanced():
        ...
        try:
            if request.method == "GET":
                ...
                history = []
                return jsonify(
                    {"history": history, "total": len(history), "filter": filter_type}
                )
