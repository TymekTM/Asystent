    ...
    if response.status_code == 200:
        ...
        if token:
            ...
            me_response = requests.get(
                "http://localhost:8001/api/v1/me", headers=headers
            )
            ...
            ai_query = {"query": "Hello, this is a test query"}
            ai_response = requests.post(
                "http://localhost:8001/api/v1/ai/query", headers=headers, json=ai_query
            )

            print(f"/ai/query Status: {ai_response.status_code}")
            print(
                f"/ai/query Response: {ai_response.text[:500]}..."
