...
from database_models import (
    # New memory system functions
    add_short_term_memory,
    get_short_term_memories,
    update_short_term_memory_access,
    delete_expired_short_term_memories,
    add_mid_term_memory,
    get_mid_term_memories,
    increment_mid_term_memory_access,
    delete_expired_mid_term_memories,
    add_long_term_memory_enhanced,
    get_long_term_memories_enhanced,
    update_long_term_memory_importance,
    ...
    add_memory as add_memory_legacy,
    get_memories as get_memories_legacy,
)
...
EMOTIONAL_KEYWORDS = {
    'positive': ['szczęśliwy', 'radosny', 'zadowolony', 'excited', 'happy', 'joy', 'love', 'great', 'amazing', 'wonderful'],
    'negative': ['smutny', 'zły', 'frustrated', 'angry', 'sad', 'upset', 'worried', 'anxious', 'stressed', 'depressed'],
    'important': ['ważne', 'important', 'critical', 'urgent', 'remember', 'pamiętaj', 'nie zapomnij', "don't forget"]
}

# Personal pronouns and relationship indicators
PERSONAL_INDICATORS = [
    'moja', 'mój', 'moje', 'my', 'mine', 'I', 'ja', 'me', 'mnie',
    'rodzina', 'family', 'żona', 'wife', 'mąż', 'husband', 'dziecko', 'child',
    'mama', 'tata', 'mother', 'father', 'brat', 'brother', 'siostra', 'sister'
]
...
class MemoryEntry:
    ...
    def from_mid_term(cls, mt_memory) -> 'MemoryEntry':
        ...
        return cls(
            ...
            context_type=ContextType.TASK if mt_memory.context_type == "task" else ContextType.CONVERSATION if mt_memory.context_type == "conversation" else None,
...
class MemoryAnalyzer:
    ...
    def analyze_importance(self, content: str, user: str, context: Dict[str, Any] = None) -> MemoryAnalysisResult:
        ...
        if context:
            factors['repetition'] = self._analyze_repetition_context(content, user, context)
        ...
        importance_score = sum(factors.get(factor, 0) * weight for factor, weight in weights.items())
    ...
    def _analyze_repetition_context(self, content: str, user: str, context: Dict[str, Any]) -> float:
    ...
    def _analyze_temporal_importance(self, content: str, context: Dict[str, Any]) -> float:
        """Analyze temporal importance (upcoming events, deadlines, etc.)."""
        # Look for temporal indicators
        temporal_keywords = ['dziś', 'today', 'jutro', 'tomorrow', 'wczoraj', 'yesterday', 
                           'wkrótce', 'soon', 'później', 'later', 'nigdy', 'never']
        
        urgent_keywords = ['pilne', 'urgent', 'natychmiast', 'immediately', 'szybko', 'quickly']
    ...
    def _recommend_memory_type(self, importance_score: float, factors: Dict[str, float]) -> MemoryType:
...
class MemorySearchEngine:
    ...
    def search_all_memories(self, query: str, limit: int = 50, user: str = None, 
                           memory_types: List[MemoryType] = None) -> MemorySearchResult:
        """Search across all memory types."""
        start_time = datetime.now()
        
        if memory_types is None:
            memory_types = [MemoryType.SHORT_TERM, MemoryType.MID_TERM, MemoryType.LONG_TERM]
        ...
        for memory in all_memories:
            relevance_scores[memory.id] = self._calculate_relevance(query, memory.content)
    ...
    def _search_memory_type(self, query: str, memory_type: MemoryType, 
                           limit: int, user: str = None) -> List[MemoryEntry]:
        """Search specific memory type."""
        memories = []
        
        try:
            if memory_type == MemoryType.SHORT_TERM:
                short_memories = get_short_term_memories(limit=limit, user=user, exclude_expired=True)
                memories = [self._convert_to_memory_entry(m, MemoryType.SHORT_TERM) for m in short_memories]
                
            elif memory_type == MemoryType.MID_TERM:
                mid_memories = get_mid_term_memories(limit=limit, user=user, exclude_expired=True)
                memories = [self._convert_to_memory_entry(m, MemoryType.MID_TERM) for m in mid_memories]
                
            elif memory_type == MemoryType.LONG_TERM:
                long_memories = get_long_term_memories_enhanced(limit=limit, query=query, user=user)
                memories = [self._convert_to_memory_entry(m, MemoryType.LONG_TERM) for m in long_memories]
    ...
    def _convert_to_memory_entry(self, memory_obj: Union[ShortTermMemory, MidTermMemory, LongTermMemory], 
                                memory_type: MemoryType) -> MemoryEntry:
        ...
        return MemoryEntry(
            ...
            created_at=memory_obj.created_at if hasattr(memory_obj, 'created_at') else datetime.now(),
    ...
    def search_memories(self, memories: List[MemoryEntry], query: str, 
                       limit: int = 50, user: str = None, 
        ...
        for memory in filtered_memories:
            ...
            if memory.context_tags:
                context_tags_str = memory.context_tags if isinstance(memory.context_tags, str) else ','.join(memory.context_tags)
...
class MemoryManager:
    ...
    def add_memory(self, content: str, user: str = "assistant", 
                  memory_type: MemoryType = None, context: Dict[str, Any] = None) -> MemoryEntry:
        ...
        logger.info(f"Added {memory_type.value} memory {memory_id} with importance {analysis.new_score:.2f}")
        return entry
    
    def get_memories(self, query: str = "", memory_types: List[MemoryType] = None, 
                    user: str = None, limit: int = 50) -> MemorySearchResult:
    ...
    def search_memories(self, query: str = "", memory_types: List[MemoryType] = None, 
        ...
        return self.get_memories(query=query, memory_types=memory_types, user=user, limit=limit)
    ...
    def cleanup_expired_memories(self) -> int:
        ...
        try:
            ...
            if total > 0:
                logger.info(f"Cleaned up {total} expired memories ({short_deleted} short, {mid_deleted} mid)")
            
            return total
        finally:
            self.cleanup_running = False
    
    def analyze_memory_patterns(self, user: str = None, days: int = 7) -> Dict[str, Any]:
        ...
        filtered_analytics = [
            ...
            if a.timestamp >= start_time and (user is None or user in str(a.context_data))
        ]
    ...
    def _store_memory(self, entry: MemoryEntry) -> int:
        ...
        if entry.memory_type == MemoryType.SHORT_TERM:
            ...
            return add_mid_term_memory(
                content=entry.content,
                user=entry.user,
                context_type=entry.context_type.value if entry.context_type else ContextType.CONVERSATION.value,
                importance_score=entry.importance_score,
                context_tags=entry.context_tags
            )
        else:  # LONG_TERM
            return add_long_term_memory_enhanced(
                ...
                memory_type=entry.context_type.value if entry.context_type else "general",
                context_tags=entry.context_tags
            )
    
    def _determine_context_type(self, content: str, context: Dict[str, Any] = None) -> ContextType:
        """Determine context type from content and context."""
        content_lower = content.lower()
        
        # Check for task-related content
        task_keywords = ['zadanie', 'task', 'praca', 'work', 'projekt', 'project', 'deadline']
        ...
        learning_keywords = ['nauczyć', 'learn', 'understand', 'rozumieć', 'explain', 'wyjaśnić']
    ...
    def _generate_context_tags(self, content: str, context: Dict[str, Any] = None) -> str:
        ...
        topic_mapping = {
            ...
            'family': ['rodzina', 'family', 'żona', 'wife', 'mąż', 'husband', 'dziecko', 'child'],
            ...
            'food': ['jedzenie', 'food', 'restauracja', 'restaurant', 'kolacja', 'dinner']
        }
        ...
        temporal_patterns = {
            ...
            'past': ['wczoraj', 'yesterday', 'wcześniej', 'earlier', 'poprzedni', 'previous']
        }
    ...
    def _should_promote_mid_to_long(self, memory: MidTermMemory) -> bool:
        ...
        if memory.context_type in important_contexts and memory.importance_score > IMPORTANCE_THRESHOLDS['medium']:
            return True
    ...
    def _promote_short_to_mid(self, memory: ShortTermMemory) -> bool:
        ...
        try:
            ...
            add_memory_analytics_entry(
                ...
                context_data=json.dumps({'from_memory_id': memory.id, 'from_type': 'short'})
            )
    ...
    def _promote_mid_to_long(self, memory: MidTermMemory) -> bool:
        ...
        try:
            ...
            add_memory_analytics_entry(
                ...
                action=AnalyticsAction.PROMOTE.value,
                context_data=json.dumps({'from_memory_id': memory.id, 'from_type': 'mid'})
            )
...
class MemoryMaintenanceTask:
    ...
    async def start_maintenance_loop(self, interval_minutes: int = 30):
        ...
        self.running = True
        logger.info(f"Starting memory maintenance loop (interval: {interval_minutes} minutes)")
    ...
    async def _run_maintenance(self):
        ...
        try:
            ...
            if results['short_to_mid'] > 0 or results['mid_to_long'] > 0 or cleaned > 0:
                logger.info(f"Memory maintenance: promoted {results['short_to_mid']} to mid, "
...
def add_memory_advanced(content: str, user: str = "assistant", 
                       memory_type: MemoryType = None, context: Dict[str, Any] = None) -> Tuple[str, bool]:
    """Add memory with advanced analysis - returns (message, success) for compatibility."""
    try:
        manager = get_memory_manager()
        entry = manager.add_memory(content, user, memory_type=memory_type, context=context)
...
def search_memories_advanced(query: str = "", user: str = None, 
                           limit: int = 50) -> List:
