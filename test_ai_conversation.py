        ...
        return {
            "choices": [{
                "message": {
                    ...
                    ] if self.function_calls else None
                }
            }]
        }
...
class AIFunctionCallingSimulator:
    ...
    async def execute_function_call(self, function_name, arguments, user_id=1):
        ...
        if not hasattr(module, 'execute_function'):
            return {"error": f"Module {module_name} does not support function execution"}
    ...
    async def simulate_ai_conversation(self, user_query, mock_ai_response):
        ...
        for i, func_call in enumerate(mock_ai_response.function_calls, 1):
            ...
            print(f"📋 Argumenty: {json.dumps(func_call['arguments'], ensure_ascii=False, indent=2)}")
...
async def main():
            ...
            'ai_response': MockOpenAIResponse(
                function_calls=[
                    ...
                    },
                    {
                        ...
                        'arguments': {'task': 'Wykonaj zaplanowane zadanie', 'priority': 'medium'}
                    }
                ],
                ...
                function_calls=[
                    {
                        ...
                        'arguments': {'query': 'artificial intelligence', 'test_mode': True}
                    },
                    {
        ...
        },
        {
            ...
            'ai_response': MockOpenAIResponse(
                function_calls=[
                    ...
                    {
                        'name': 'core_module_get_current_time',
                        'arguments': {}
                    }
                ],
    ...
    successful_calls = sum(
        sum(1 for func_result in result['result']['function_results'] 
