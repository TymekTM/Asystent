...
def get_audio_input_devices():
    ...
    if not SOUNDDEVICE_AVAILABLE:
        devices.append(
            {"id": "error", "name": "SoundDevice not available", "is_default": False}
        )
        return devices

    try:
        sd_devices = sd.query_devices()
        default_input_device_index = sd.default.device[
            0
        ...
        for i, device in enumerate(sd_devices):
            if device["max_input_channels"] > 0:
                ...
                devices.append(
                    {"id": i, "name": device["name"], "is_default": is_default}
                )
    except Exception as e:
        logger.error(f"Error querying audio devices: {e}")
        # Optionally return a default/error indicator
        devices.append(
            {"id": "error", "name": "Nie można pobrać urządzeń", "is_default": False}
        )
    return devices


def get_audio_output_devices():
    """Gets a list of available audio output devices with IDs and names."""
    devices = []
    if not SOUNDDEVICE_AVAILABLE:
        devices.append(
            {"id": "error", "name": "SoundDevice not available", "is_default": False}
    ...
    try:
        sd_devices = sd.query_devices()
        default_output_device_index = sd.default.device[
            1
        ...
        devices.append(
            {
                "id": "error",
                "name": "Nie można pobrać urządzeń wyjściowych",
                "is_default": False,
            }
        )
    return devices
...
_assistant_instance = (
    None  # defer Assistant instantiation to avoid ASR load errors on startup
)
