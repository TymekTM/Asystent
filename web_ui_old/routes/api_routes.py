...
from utils.benchmark_manager import get_benchmark_status
from utils.benchmark_manager import run_assistant_benchmarks as start_benchmarks
...
def setup_api_routes(app, assistant_queue):
        ...
        try:
            ...
            if assistant_queue:
                try:
                    ...
                    logger.info(
                        "Sent reload_config and shutdown commands to assistant process."
                    )
            ...
            return (
                jsonify(
                    [
                        {
                            "id": "-1",
                            "name": f"Error retrieving devices: {e}",
                            "is_default": False,
                        }
                    ]
                ),
    ...
    def api_speakers():
        """Return list of available speaker/output devices."""
        try:
            ...
            return (
                jsonify(
                    [
                        {
                            "id": "-1",
                            "name": f"Error retrieving output devices: {e}",
                            "is_default": False,
                        }
                    ]
                ),
    ...
    def api_history():
        ...
        if request.method == "DELETE":
            logger.warning(
                f"Request to clear history received from user '{session.get('username')}'."
            )
            try:
                ...
                logger.error(
                    f"Unexpected error during history clearing: {e}", exc_info=True
                )
    ...
    def stop_assistant():
        ...
        lock_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "assistant_stop.lock"
        )
        with open(lock_path, "w") as f:
            f.write("stop")
    ...
    def api_long_term_memory():
        ...
        logger.info(
            f"LTM API endpoint accessed (Method: {request.method}) by user '{session.get('username')}'."
        )
        if request.method == "POST":
            entry = request.json
    ...
    def delete_memory_api(memory_id):
        try:
            ...
            if success:
                logger.info(
                    f"Memory {memory_id} deleted via API by {session.get('username')}"
                )
                return jsonify({"success": True})
            else:
                logger.warning(
                    f"Attempt to delete non-existent memory {memory_id} via API"
                )
        ...
        if request.method == "DELETE":
            if "username" in session and session.get("role") == "dev":
                if clear_performance_stats():
                    logger.info(
                        f"Performance stats cleared by user '{session['username']}'."
                    )
                    return (
                        jsonify({"message": "Performance stats cleared successfully."}),
                        200,
                    )
                else:
    ...
    def api_run_benchmarks():
        ...
        try:
            ...
            return (
                jsonify(
                    {"status": "started", "message": "Benchmarks started successfully."}
                ),
                200,
            )
    ...
    def api_run_tests():
        ...
        try:
            ...
            return (
                jsonify(
                    {"status": "started", "message": "Tests started successfully."}
                ),
                200,
            )
        ...
        try:
            ...
            if config.get("BENCHMARK_SCHEDULE", {}).get("enabled", False):
                status.update(
                    {
                        "active": True,
                        "interval": config["BENCHMARK_SCHEDULE"].get(
                            "interval_minutes", 60
                        ),
            ...
            ), 500 @ app.route("/api/test_history", methods=["GET"])
    ...
    def api_complete_onboarding():
        ...
        try:
            ...
            if success:
                ...
                logger.info(
                    "Onboarding completed successfully via API, dev user auto-logged in"
                )
                return (
                    jsonify(
                        {
            ...
            while True:
                try:
                    ...
                    if shared_state:
                        ...
                        wake_word_detected = shared_state.get(
                            "wake_word_detected", False
                        )
                    else:
                        assistant = get_assistant_instance()
                        is_listening = (
                            getattr(assistant, "is_listening", False)
                            if assistant
                            else False
                        )
                        is_speaking = (
                            getattr(assistant, "is_speaking", False)
                            if assistant
                            else False
                        )
                        last_text = (
                            getattr(assistant, "last_tts_text", "") if assistant else ""
                        )
                        wake_word_detected = (
                            getattr(assistant, "wake_word_detected", False)
                            if assistant
                            else False
                        )
                    ...
                    if current_status != last_status or not last_status:
                        ...
                        if (
                            current_status.get("is_listening")
                            or current_status.get("is_speaking")
                            or current_status.get("wake_word_detected")
                        ):
