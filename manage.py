...
def build_client():
    ...
    return run_command([sys.executable, "build.py", "--component", "client"], "Building client")

def docker_build_server():
    """Build server Docker image."""
    print("[DOCKER] Building server Docker image...")
    return run_command(["docker-compose", "build", "gaja-server-cpu"], "Building Docker image")

def docker_start_server():
    """Start server in Docker."""
    print("[DOCKER] Starting server...")
    return run_command(["docker-compose", "up", "-d", "gaja-server-cpu"], "Starting server")
...
def docker_logs_server():
    ...
    return run_command(["docker-compose", "logs", "-f", "gaja-server-cpu"], "Server logs")
...
def main():
    ...
    parser.add_argument('action', choices=[
        'build-client', 'build-server', 'start-server', 'stop-server', 
        'logs', 'status', 'test', 'setup', 'help'
