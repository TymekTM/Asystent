...
class WakewordDetector:
    ...
    def __init__(self, config: Dict[str, Any], callback: Callable):
        ...
        logger.info(f"WakewordDetector adapter initialized for keyword '{self.keyword}' with sensitivity {self.sensitivity}")
    ...
    async def start(self):
        ...
        self.detection_thread = threading.Thread(
            target=self._run_detection_wrapper,
            daemon=True
        )
    ...
    def start_detection(self):
        ...
        if not self.is_running:
            ...
            self.is_running = True
            self.detection_thread = threading.Thread(target=self._run_detection_wrapper, daemon=True)
            self.detection_thread.start()
        else:
...
def create_wakeword_detector(config: Dict[str, Any], callback: Callable) -> WakewordDetector:
    """
