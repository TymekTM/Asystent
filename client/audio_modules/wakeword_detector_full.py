...
def get_base_path():
    """Returns the base path for resources, whether running after bundle or in development."""
    if getattr(sys, 'frozen', False):
        # In bundled mode, resources live next to the launcher executable
        return BASE_DIR
    # Development mode: two levels up from this file's directory
...
def run_wakeword_detection(
            ...
            for i, device in enumerate(default_devices):
                ...
                if device['max_input_channels'] > 0: 
                    try:
                        ...
                        if 'default_input_device_name' in host_api_info and device['name'] == host_api_info['default_input_device_name']:
                            default_input_device_index = i
                            logger.info(f"Found host API default input device: ID {i} ({device['name']})")
                            break
                    except KeyError:
    ...
    base_project_dir = get_base_path() # Use the new helper function to get the project's base directory
    ...
    try:
        ...
        logger.error("openwakeword library is required. Install via `pip install openwakeword`.", exc_info=True)
        play_beep("error", loop=False)
        return
    except Exception as e:
        logger.error(f"Error initializing openWakeWord Model: {e}", exc_info=True)
    ...
    try:
        ...
        with audio_stream:
            logger.info(f"Audio stream started on device ID {mic_device_id} for wake word detection with chunk size {oww_chunk_size}.")
            logger.info("🔄 Starting main wakeword detection loop...")
            while not stop_detector_event.is_set():
                try:
                    # Debug: Log every few iterations to confirm loop is running
                    import time
                    current_time = time.time()
                    if not hasattr(run_wakeword_detection, 'last_debug_time'):
                        run_wakeword_detection.last_debug_time = current_time
                    if current_time - run_wakeword_detection.last_debug_time > 5:  # Every 5 seconds
                        logger.debug("🔄 Wakeword detection loop is running...")
                        run_wakeword_detection.last_debug_time = current_time
                    ...
                    if manual_listen_trigger_event.is_set():
                        ...
                        # Note: In client-server architecture, we don't have a local assistant instance
                        logger.info("🎙️ Manual trigger detected - starting command recording")
                        
                        # Stop the current audio stream temporarily to avoid conflicts
                        audio_stream.stop()
                        logger.info("🔇 Temporarily stopped wakeword audio stream for command recording")
                        ...
                        audio_stream.start()
                        logger.info("🔊 Restarted wakeword audio stream")

                        logger.info(f"📊 Audio recording result: {command_audio_data_np is not None}, size: {command_audio_data_np.size if command_audio_data_np is not None else 'None'}")
                        ...
                        if command_audio_data_np is not None and command_audio_data_np.size > 0:
                            if whisper_asr_instance:
                                ...
                                logger.info(f"📊 Audio stats: Max amp={np.max(np.abs(audio_to_transcribe)):.4f}, Mean amp={np.mean(np.abs(audio_to_transcribe)):.4f}, dtype={audio_to_transcribe.dtype}, shape={audio_to_transcribe.shape}, duration={len(audio_to_transcribe)/SAMPLE_RATE:.2f}s")
                                
                                # Check if audio has actual content
                                if np.max(np.abs(audio_to_transcribe)) < 0.001:
                                ...
                                logger.info(f"🗣️ Whisper transcription (manual): '{transcribed_text}'")
                                if transcribed_text and transcribed_text.strip():
                                    logger.info("🚀 Calling process_query_callback_async...")
                                    asyncio.run_coroutine_threadsafe(process_query_callback_async(transcribed_text.strip()), async_event_loop)
                                    logger.info("✅ process_query_callback_async called successfully")
                                else:
                        ...
                        logger.debug("✅ Manual trigger processing completed")
                    ...
                    for model_name_key, score_value in prediction_scores.items():
                        if score_value >= oww_sensitivity_threshold:
                            ...
                            logger.debug("Wake word processing completed")
                            if wakeword_model_instance: wakeword_model_instance.reset()
                            break
        ...
        logger.error(f"PortAudioError during audio stream setup: {pae}", exc_info=True)
        play_beep("error", loop=False)
    except Exception as e:
