...
Uses heavy ML dependencies for advanced wake word detection.
"""
...
def run_wakeword_detection(
    ...
    whisper_asr: Optional[Any] = None,
    manual_listen_event: Optional[threading.Event] = None,
    ...
    """
    logger.info(f"Starting full wakeword detection for '{keyword}' (sensitivity: {sensitivity})")
    
    if not _check_heavy_dependencies():
        logger.warning("Heavy dependencies not available - falling back to basic detection")
...
def _run_advanced_detection(
    ...
    def audio_callback(indata, frames, time, status):
        ...
        try:
            ...
            if amplitude > VOICE_THRESHOLD or (manual_listen_event and manual_listen_event.is_set()):
    ...
    # Start audio stream
    logger.info(f"Starting audio stream (device: {device_id}, sample_rate: {SAMPLE_RATE})")
            ...
            if amplitude > VOICE_THRESHOLD or (manual_listen_event and manual_listen_event.is_set()):
                ...
                if len(audio_buffer) >= 10:
                    if _simple_keyword_detection(audio_buffer, sensitivity):
                        logger.info(f"Wake word detected (basic method)")
                        if process_query_callback:
                            _trigger_callback(process_query_callback, "", event_loop)
                        audio_buffer.clear()
            else:
