"""
Memory module for GAJA Assistant
Simple database-backed memory system for the server.
"""
...
class MemoryModule:
    ...
    async def store_memory(self, user_id: str, key: str, value: Any, category: str = "general") -> bool:
        ...
        try:
            ...
            await self.db_manager.save_interaction(user_id, f"MEMORY_STORE:{key}", str(value))
    ...
    async def get_memory(self, user_id: str, key: str) -> Optional[str]:
        ...
        try:
            ...
            for item in history:
                if item['role'] == 'user' and item['content'].startswith(f"MEMORY_STORE:{key}"):
...
async def store_memory(user_id: str, key: str, value: str, category: str = "general") -> Dict[str, Any]:
    """Store a memory item."""
    try:
        from server_main import server_app
        success = await server_app.memory_module.store_memory(user_id, key, value, category)
        return {
            "success": success,
