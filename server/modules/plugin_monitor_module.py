"""
Plugin Monitor Module for Plugin System
Moduł monitorowania pluginów dostępny przez system pluginów.
"""
...
class PluginMonitorModule:
    ...
    async def reload_plugin(self, plugin_name: str) -> Dict[str, Any]:
        ...
        try:
            if hasattr(server_app, 'plugin_monitor'):
                result = await server_app.plugin_monitor.reload_plugin_manually(plugin_name)
                return result
            return {"success": False, "error": "Plugin monitor not available"}
        except Exception as e:
