...
class WeatherModule:
    ...
    async def get_weather(self, user_id: int, location: str,
                         api_key: str, provider: str = "openweather") -> Dict[str, Any]:
        """
    ...
    async def _get_openweather_data(self, user_id: int, location: str, 
        ...
        current_response = await self.api_module.get(user_id, current_url, params=current_params)
        ...
        forecast_response = await self.api_module.get(user_id, forecast_url, params=forecast_params)
        forecast_data = forecast_response.get('data', {}) if forecast_response.get('status') == 200 else {}
        
        # Przetwórz dane
        weather_info = self._process_openweather_data(current_data, forecast_data, location)
        weather_info['provider'] = 'openweather'
        weather_info['timestamp'] = datetime.now().isoformat()
        
        return weather_info
    
    def _process_openweather_data(self, current_data: Dict, forecast_data: Dict, 
        ...
        if forecast_data.get('list'):
            ...
            for date_str, day_data in daily_forecasts.items():
                ...
                forecast.append({
                    ...
                    'description': max(set(day_data['descriptions']), key=day_data['descriptions'].count) if day_data['descriptions'] else '',
                    'avg_humidity': round(sum(day_data['humidity']) / len(day_data['humidity'])) if day_data['humidity'] else 0,
                    'avg_wind_speed': round(sum(day_data['wind_speed']) / len(day_data['wind_speed']), 1) if day_data['wind_speed'] else 0,
                    'hourly': day_data['hourly'][:8]  # Tylko pierwsze 8 godzin na dzień
                })
    ...
    async def _get_weatherapi_data(self, user_id: int, location: str, 
                                  api_key: str) -> Dict[str, Any]:
    ...
    def _process_weatherapi_data(self, data: Dict, location: str) -> Dict[str, Any]:
        ...
        current_weather = {
            ...
            'air_quality': self._process_air_quality(data.get('current', {}).get('air_quality', {}))
        }
    ...
    async def get_weather_advice(self, user_id: int, weather_data: Dict[str, Any]) -> str:
        """
        ...
        if temp < 0:
            advice_parts.append("🧥 Na zewnątrz jest bardzo zimno - załóż ciepłą kurtkę, czapkę i rękawiczki!")
        elif temp < 10:
            ...
            advice_parts.append("🌡️ Jest bardzo gorąco - noś lekkie ubrania i pamiętaj o nawodnieniu!")
        ...
        if wind_speed > 10:
            advice_parts.append("💨 Silny wiatr - uważaj na spadające gałęzie i trzymaj mocno parasol!")
        ...
        if uv_index > 6:
            advice_parts.append("☀️ Wysokie promieniowanie UV - używaj kremu z filtrem!")
        
        return "\n".join(advice_parts) if advice_parts else "Pogoda wygląda na stabilną - ubierz się zgodnie z temperaturą!"
...
async def get_weather_module() -> WeatherModule:
    ...
    async def get_forecast(self, user_id: int, location: str, api_key: str, days: int = 3) -> Dict[str, Any]:
        """
    ...
    def _process_forecast_data(self, data: Dict, location: str, days: int) -> Dict[str, Any]:
        """
        ...
        for item in forecast_list:
            ...
            daily_forecasts[date_str]['humidity'].append(item['main']['humidity'])
            daily_forecasts[date_str]['wind_speed'].append(item['wind']['speed'])
            daily_forecasts[date_str]['precipitation'].append(hourly_data['precipitation'])
