"""
Enhanced Plugin Manager for GAJA Assistant Server
Manages dynamic loading/unloading of plugins per user.
"""
...
class PluginManager:
    ...
    async def _analyze_plugin(self, file_path: Path) -> Optional[PluginInfo]:
    ...
    async def unload_plugin(self, plugin_name: str) -> bool:
        ...
        try:
            ...
            functions_to_remove = [
                func_name for func_name in self.function_registry.keys()
    ...
    async def call_plugin_function(self, user_id: str, plugin_name: str, function_name: str, parameters: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            if hasattr(plugin_info.module, 'execute_function'):
                result = await plugin_info.module.execute_function(function_name, parameters, int(user_id))
                ...
                return {"success": False, "error": f"Plugin {plugin_name} has no execute_function method"}
    ...
    async def reload_plugin(self, plugin_name: str) -> bool:
        ...
        enabled_users = [
            user_id for user_id, plugins in self.user_plugins.items()
        ...
        plugin_info = await self._analyze_plugin(Path(self.plugins[plugin_name].module_path))
        if plugin_info:
