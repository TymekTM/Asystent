"""
Routines Learner Module for Asystent
AI system wykrywający i analizujący powtarzalne wzorce dnia użytkownika.
"""
...
class RoutinesLearnerModule:
    ...
    def __init__(self, config: Dict[str, Any], db_manager=None):
        ...
        self.min_pattern_frequency = config.get('routines_learner', {}).get('min_pattern_frequency', 3)
        self.pattern_similarity_threshold = config.get('routines_learner', {}).get('similarity_threshold', 0.8)
        self.learning_window_days = config.get('routines_learner', {}).get('learning_window_days', 14)
        self.prediction_confidence_threshold = config.get('routines_learner', {}).get('prediction_threshold', 0.7)
        ...
        self.routine_templates = {
            'morning': {
                'time_range': (6, 12),
                'typical_activities': ['start_session', 'first_interaction', 'check_briefing'],
                'duration_range': (30, 180)  # minutes
            },
            'work_block': {
                'time_range': (9, 18),
                'typical_activities': ['focused_work', 'application_usage', 'productivity'],
    ...
    async def _save_activity_sequences(self):
        ...
        try:
            ...
            filtered_sequences = [
                seq for seq in self.activity_sequences
                if datetime.fromisoformat(seq.get('start_time', '1970-01-01')) > cutoff_date
            ]
    ...
    async def _finalize_current_sequence(self):
        ...
        sequence_data = {
            ...
            'duration_minutes': (datetime.now() - self.sequence_start_time).total_seconds() / 60,
        ...
        logger.debug(f"Finalized sequence: {len(self.current_sequence)} activities, "
    ...
    async def record_activity(self, activity_type: str, details: Dict = None):
        ...
        if len(self.current_sequence) > 1:
            ...
            time_gap = (datetime.now() - datetime.fromisoformat(last_activity['timestamp'])).total_seconds() / 60
    ...
    async def _detect_time_based_patterns(self, sequences: List[Dict]):
        ...
        for hour, hour_sequences in sequences_by_hour.items():
            if len(hour_sequences) >= self.min_pattern_frequency:
                ...
                activity_types = [act['type'] for seq in hour_sequences for act in seq['activities']]
                common_activities = Counter(activity_types).most_common(5)
                
                # Calculate consistency
                total_days = len(set(datetime.fromisoformat(seq['start_time']).date() for seq in hour_sequences))
                ...
                if frequency >= 0.5:  # Occurs at least 50% of the time
                    pattern = {
                        ...
                        'average_duration': statistics.mean([seq['duration_minutes'] for seq in hour_sequences]),
    ...
    async def _detect_activity_patterns(self, sequences: List[Dict]):
        ...
        self.detected_routines['activity_patterns'] = sequence_patterns[:20]  # Keep top 20
        ...
        for day, day_sequences in sequences_by_day.items():
            if len(day_sequences) >= 3:  # At least 3 occurrences
                ...
                pattern = {
                    ...
                    'frequency': len(day_sequences) / max(1, len(sequences) / 7),  # Normalized frequency
                    'consistency': 1 - (statistics.stdev(hours) / 12 if len(hours) > 1 else 0)  # Hour consistency
                }
        ...
        try:
            ...
            for seq in sequences:
                feature_vector = [
                    ...
                    seq['activity_count'],
                    seq['day_of_week'],
                    len(set(act['type'] for act in seq['activities']))  # Unique activity types
                ]
            ...
            for cluster_id in range(n_clusters):
                cluster_sequences = [seq for i, seq in enumerate(sequences) if cluster_labels[i] == cluster_id]
                
                if len(cluster_sequences) >= 2:
                    # Calculate cluster characteristics
                    avg_start_hour = statistics.mean([seq['start_hour'] for seq in cluster_sequences])
                    avg_duration = statistics.mean([seq['duration_minutes'] for seq in cluster_sequences])
                    common_activities = Counter([
                        act['type'] for seq in cluster_sequences for act in seq['activities']
                    ]).most_common(5)
    ...
    async def _ai_routine_analysis(self, sequences: List[Dict]):
        ...
        try:
            ...
            data_summary = {
                ...
                'average_duration': statistics.mean([seq['duration_minutes'] for seq in sequences]),
                'most_common_activities': Counter([
                    act['type'] for seq in sequences for act in seq['activities']
                ]).most_common(10),
                'days_covered': len(set(datetime.fromisoformat(seq['start_time']).date() for seq in sequences))
            }
...
"""
    ...
    async def get_predictions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for pattern in time_patterns:
                if pattern['hour'] > current_hour and pattern['confidence'] > self.prediction_confidence_threshold:
                    predictions[f"activity_at_{pattern['hour']}"] = {
                        'type': 'time_based',
                        'hour': pattern['hour'],
                        'expected_activities': [act[0] for act in pattern['common_activities'][:3]],
    ...
    async def _generate_routine_recommendations(self) -> List[str]:
        ...
        try:
            ...
            weekly_patterns = self.detected_routines.get('weekly_patterns', [])
            if weekly_patterns:
                most_productive_day = max(weekly_patterns, key=lambda x: x['average_activities'])
                day_names = ['poniedziałek', 'wtorek', 'środa', 'czwartek', 'piątek', 'sobota', 'niedziela']
                recommendations.append(
            ...
            if activity_patterns:
                strongest_pattern = max(activity_patterns, key=lambda x: x['confidence'])
                recommendations.append(
    ...
    def _get_analysis_suggestions(self) -> List[str]:
        ...
        if len(recent_sequences) < 20:
            suggestions.append("Zbierz więcej danych (aktualnie: " + str(len(recent_sequences)) + " sekwencji) dla lepszej analizy wzorców.")
        
        if not SKLEARN_AVAILABLE:
            suggestions.append("Zainstaluj scikit-learn dla zaawansowanej analizy wzorców ML.")
        ...
        if len(unique_activities) < 5:
            suggestions.append("Zróżnicuj aktywności dla bogatszego modelowania wzorców.")
...
async def record_routine_activity(user_id: str, activity_type: str, details: Dict = None) -> Dict[str, Any]:
