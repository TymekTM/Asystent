"""
Day Summary Module for Asystent
Zbiera dane o aktywności użytkownika przez cały dzień i tworzy podsumowania.
"""
...
class DaySummaryModule:
    ...
    def __init__(self, config: Dict[str, Any], db_manager=None):
        ...
        self.track_applications = config.get('day_summary', {}).get('track_applications', True)
        self.track_interactions = config.get('day_summary', {}).get('track_interactions', True)
        self.track_productivity = config.get('day_summary', {}).get('track_productivity', True)
        ...
        self.productivity_categories = {
            'high_productivity': ['vscode', 'visual studio code', 'pycharm', 'intellij', 'sublime', 'atom', 'notepad++', 'vim', 'emacs'],
            'medium_productivity': ['browser', 'chrome', 'firefox', 'edge', 'office', 'word', 'excel', 'powerpoint', 'outlook'],
            'low_productivity': ['youtube', 'facebook', 'twitter', 'instagram', 'tiktok', 'netflix', 'games'],
            'communication': ['discord', 'slack', 'teams', 'zoom', 'skype', 'telegram', 'whatsapp'],
        ...
        try:
            ...
            if current_app != self.current_application:
                ...
                if self.current_application and self.application_start_time:
                    ...
                    self.current_day_data['applications'][self.current_application] += duration
    ...
    def _check_inactivity(self):
        ...
        now = datetime.now()
        time_since_activity = (now - self.last_activity_time).total_seconds() / 60  # minutes
        ...
        if time_since_activity > self.inactive_threshold_minutes:
            ...
            if self.current_application:
                ...
                if self.application_start_time:
                    duration = (self.last_activity_time - self.application_start_time).total_seconds()
                    self.current_day_data['applications'][self.current_application] += duration
            ...
            if not self.current_day_data['breaks'] or \
               (datetime.fromisoformat(self.current_day_data['breaks'][-1]['end']) + timedelta(minutes=self.inactive_threshold_minutes)) < self.last_activity_time:
    ...
    async def _save_current_day_data(self):
        ...
        try:
            ...
            self.current_day_data['productivity_score'] = self._calculate_productivity_score()
    ...
    async def generate_day_summary(self, date: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            total_active_time = sum(day_data.get('applications', {}).values()) / 3600  # hours
            total_interactions = len(day_data.get('interactions', []))
            total_breaks = len(day_data.get('breaks', []))
            break_time = sum(break_data.get('duration_minutes', 0) for break_data in day_data.get('breaks', [])) / 60  # hours
            productivity_score = day_data.get('productivity_score', 0.0)
            
            # Top applications
            applications = day_data.get('applications', {})
            top_apps = sorted(applications.items(), key=lambda x: x[1], reverse=True)[:5]
            top_apps_formatted = [(app, duration/3600) for app, duration in top_apps]  # Convert to hours
            ...
            peak_hour = max(interactions_by_hour.items(), key=lambda x: x[1])[0] if interactions_by_hour else None
    ...
    async def _generate_ai_summary(self, summary_data: Dict) -> Optional[str]:
    ...
    async def get_week_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if days_with_data > 0:
                ...
                total_stats['average_active_time_per_day'] = total_stats['total_active_time'] / days_with_data
                total_stats['average_interactions_per_day'] = total_stats['total_interactions'] / days_with_data
...
async def record_user_interaction(user_id: str, interaction_type: str, content: str = "", metadata: Dict = None) -> Dict[str, Any]:
    """Zapisz interakcję użytkownika."""
    try:
        from server_main import server_app
        if hasattr(server_app, 'day_summary'):
            await server_app.day_summary.record_interaction(interaction_type, content, metadata)
