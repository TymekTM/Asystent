"""
Plugin Monitor System for GAJA Assistant Server
Automatyczne monitorowanie i przeładowywanie pluginów.
"""
...
class PluginFileHandler(FileSystemEventHandler):
    ...
    def __init__(self, plugin_monitor: 'PluginMonitor'):
        ...
        self.reload_delay = 2.0  # Sekundy opóźnienia dla uniknięcia wielokrotnych przeładowań
...
class PluginMonitor:
    ...
    def schedule_reload(self, file_path: Path, is_new: bool = False):
        ...
        try:
            ...
            logger.info(f"Scheduling {'new plugin' if is_new else 'reload'} for: {plugin_name}")
    ...
    def schedule_unload(self, file_path: Path):
        ...
        try:
            ...
            asyncio.create_task(self.unload_queue.put({
                'action': 'unload',
                'plugin_name': plugin_name,
                'file_path': str(file_path),
                'timestamp': time.time()
    ...
    async def _handle_reload(self, task: Dict[str, Any]):
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"Successfully {'loaded' if is_new else 'reloaded'} plugin: {plugin_name}")
            else:
                self.stats['failed_reloads'] += 1
                logger.error(f"Failed to {'load' if is_new else 'reload'} plugin: {plugin_name}")
            
            self.stats['last_reload_time'] = time.time()
            
        except Exception as e:
            self.stats['failed_reloads'] += 1
            logger.error(f"Error {'loading' if is_new else 'reloading'} plugin {plugin_name}: {e}")
    ...
    async def reload_plugin_manually(self, plugin_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not file_path.exists():
                return {"success": False, "error": f"Plugin file not found: {file_path}"}
...
async def start_plugin_monitoring() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": success,
            "message": "Plugin monitoring started" if success else "Failed to start plugin monitoring"
        }
    except Exception as e:
...
async def stop_plugin_monitoring() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": True,
            "message": "Plugin monitoring stopped"
        }
    except Exception as e:
        logger.error(f"Error stopping plugin monitoring: {e}")
        return {"success": False, "error": str(e)}
