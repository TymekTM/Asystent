...
# Import API routes
from api.routes import router as api_router, set_server_app
        ...
        try:
            ...
            for user in users:
                ...
                for plugin in plugins:
                    if plugin['enabled']:
                        await plugin_manager.enable_plugin_for_user(plugin['plugin_name'], user['user_id'])
                        logger.info(f"Plugin {plugin['plugin_name']} enabled for user {user['user_id']}")
        except Exception as e:
