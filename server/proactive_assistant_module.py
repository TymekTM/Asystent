...
class ProactiveAssistantModule:
        ...
        try:
            with self.lock:
                ...
                if "session_duration" in context_data:
                    self.user_context.session_duration = context_data["session_duration"]
                if "daily_goals_progress" in context_data:
                    self.user_context.daily_goals_progress = context_data["daily_goals_progress"]
    ...
    def wellness_check(self):
        ...
        try:
            ...
            if self.user_context.session_duration > self.config["wellness_thresholds"]["break_needed_after"]:
            ...
            if self.user_context.energy_level < self.config["wellness_thresholds"]["low_energy_threshold"]:
                self._create_notification(
                    "wellness",
                    "medium",
            ...
            if self.user_context.focus_level < self.config["wellness_thresholds"]["low_focus_threshold"]:
                self._create_notification(
                    "wellness",
                    "low",
                    "Spadek koncentracji",
    ...
    async def add_notification(self, user_id: str, notification_type: str, message: str, 
                             priority: str = "medium", title: str = None, 
                             suggested_action: str = None, context: Dict = None):
