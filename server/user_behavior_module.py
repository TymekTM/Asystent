"""
User Behavior Learning Module for Asystent
System uczący się nawyków i wzorców zachowań użytkownika.
"""
...
class UserBehaviorModule:
    ...
    def __init__(self, config: Dict[str, Any], db_manager=None):
        ...
        self.learning_period_days = config.get('user_behavior', {}).get('learning_period_days', 30)
        self.min_data_points = config.get('user_behavior', {}).get('min_data_points', 5)
        self.confidence_threshold = config.get('user_behavior', {}).get('confidence_threshold', 0.7)
            ...
            for key in self.behavior_data:
                if isinstance(self.behavior_data[key], list):
                    self.behavior_data[key] = [
                        item for item in self.behavior_data[key]
                        if datetime.fromisoformat(item.get('timestamp', '1970-01-01')) > cutoff_date
                    ]
    ...
    async def record_interaction(self, interaction_type: str, content: str = "", metadata: Dict = None):
        ...
        logger.debug(f"Recorded interaction: {interaction_type} at {interaction['hour']}:xx")
    ...
    async def record_break(self, duration_minutes: float):
        ...
        logger.debug(f"Recorded break: {duration_minutes} minutes at {break_data['hour']}:xx")
    ...
    async def _learn_daily_routines(self):
        ...
        if len(wake_times) >= self.min_data_points:
            # Analyze wake times
            wake_hours = [datetime.fromisoformat(item['timestamp']).hour for item in wake_times[-30:]]  # Last 30 days
            ...
            self.learned_patterns['daily_routines']['average_wake_time'] = {
                ...
                'consistency': max(0, 1 - (wake_time_std / 4)),  # Normalize by 4-hour range
                'data_points': len(wake_hours)
            }
        
        if len(work_end_times) >= self.min_data_points:
            # Analyze work end times
            end_hours = [datetime.fromisoformat(item['timestamp']).hour for item in work_end_times[-30:]]
    ...
    async def _learn_work_patterns(self):
        ...
        if len(sessions) >= self.min_data_points:
            ...
            work_day_preferences = {
                day: count / total_sessions
                for day, count in work_days.items()
            }
            ...
            self.learned_patterns['work_patterns'] = {
                ...
                'most_active_days': sorted(work_days.items(), key=lambda x: x[1], reverse=True)[:3]
            }
    ...
    async def _learn_break_patterns(self):
        ...
        if len(breaks) >= self.min_data_points:
            ...
            break_durations = [break_data['duration_minutes'] for break_data in breaks[-100:]]
            ...
            avg_breaks_per_day = {
                day: len(day_breaks) / max(1, len(set(item['timestamp'][:10] for item in day_breaks)))
            ...
            self.learned_patterns['break_patterns'] = {
                ...
                'break_frequency': len(breaks) / max(1, len(set(item['timestamp'][:10] for item in breaks)))
            }
    ...
    async def _learn_interaction_patterns(self):
        ...
        if len(interactions) >= self.min_data_points:
            ...
            interaction_hours = [interaction['hour'] for interaction in interactions[-200:]]
            ...
            interaction_types = [interaction['type'] for interaction in interactions[-200:]]
            ...
            avg_daily_interactions = statistics.mean(interactions_by_date.values()) if interactions_by_date else 0
    ...
    async def _learn_application_preferences(self):
        ...
        if len(app_usage) >= self.min_data_points:
            ...
            top_apps_by_time = sorted(app_time.items(), key=lambda x: x[1], reverse=True)[:10]
            
            # Most frequently used applications
            top_apps_by_frequency = sorted(app_frequency.items(), key=lambda x: x[1], reverse=True)[:10]
    ...
    async def _learn_time_preferences(self):
                ...
                for activity in activities:
                    if 'timestamp' in activity:
                        all_activities.append({
                            ...
                            'hour': activity.get('hour', datetime.fromisoformat(activity['timestamp']).hour),
                            'type': activity_type
                        })
        ...
        if len(all_activities) >= self.min_data_points:
            ...
            if activity_by_hour:
                ...
                peak_hours = [hour for hour, count in activity_by_hour.items() if count > max_activity * 0.7]
                
                # Morning/afternoon/evening preferences
                morning_activity = sum(count for hour, count in activity_by_hour.items() if 6 <= hour <= 12)
                afternoon_activity = sum(count for hour, count in activity_by_hour.items() if 12 <= hour <= 18)
                evening_activity = sum(count for hour, count in activity_by_hour.items() if 18 <= hour <= 24)
                
                total_activity = morning_activity + afternoon_activity + evening_activity
                ...
                if total_activity > 0:
                    ...
                    preferred_time = max(time_preferences.items(), key=lambda x: x[1])[0]
        ...
        if len(productivity_data) >= self.min_data_points:
            ...
            if avg_productivity_by_hour:
                most_productive_hours = sorted(
                    avg_productivity_by_hour.items(),
                    key=lambda x: x[1],
                    reverse=True
                )[:5]
                
                most_productive_days = sorted(
                    avg_productivity_by_day.items(),
                    key=lambda x: x[1],
                    reverse=True
                )[:3]
                ...
                self.learned_patterns['productivity_patterns'] = {
                    ...
                    'overall_average': statistics.mean([score['score'] for score in productivity_data[-50:]])
                }
            ...
            for pattern_name, pattern_data in patterns.items():
                if isinstance(pattern_data, dict):
                    ...
                    if data_points >= self.min_data_points:
                        confidence += min(1.0, data_points / (self.min_data_points * 2)) * 0.5
    ...
    async def get_predictions(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if break_patterns and self.pattern_confidence.get('break_patterns', 0) > self.confidence_threshold:
                common_break_hours = break_patterns.get('common_break_hours', [])
                if common_break_hours:
                    next_break_hours = [hour for hour, _ in common_break_hours if hour > current_hour]
                    if next_break_hours:
            ...
            productivity_patterns = self.learned_patterns.get('productivity_patterns', {})
            if productivity_patterns and self.pattern_confidence.get('productivity_patterns', 0) > self.confidence_threshold:
                productivity_by_hour = productivity_patterns.get('productivity_by_hour', {})
            ...
            if daily_routines and self.pattern_confidence.get('daily_routines', 0) > self.confidence_threshold:
        ...
        try:
            insights = {
                ...
                'data_summary': {
                    ...
                    'total_interactions': len(self.behavior_data.get('interaction_times', [])),
                    'total_app_usage_records': len(self.behavior_data.get('application_usage', [])),
    ...
    async def _generate_recommendations(self) -> List[str]:
        ...
        try:
            ...
            if productivity_patterns:
                most_productive_hours = productivity_patterns.get('most_productive_hours', [])
            ...
            if break_patterns:
                ...
                if avg_break_duration > 30:
                    recommendations.append(
                        "Twoje przerwy są dość długie (średnio {:.1f} min). "
                        "Rozważ krótsze, ale częstsze przerwy dla lepszej produktywności.".format(avg_break_duration)
                    )
