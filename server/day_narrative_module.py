"""
Day Narrative Module for Asystent
System generujący narracyjne podsumowania dnia używając AI.
"""
...
class DayNarrativeModule:
    ...
    def __init__(self, config: Dict[str, Any], db_manager=None):
        ...
        self.narrative_style = config.get('day_narrative', {}).get('style', 'friendly')  # friendly, professional, casual, poetic
        self.narrative_length = config.get('day_narrative', {}).get('length', 'medium')  # short, medium, long
        self.include_emotions = config.get('day_narrative', {}).get('include_emotions', True)
        self.include_insights = config.get('day_narrative', {}).get('include_insights', True)
        self.include_comparisons = config.get('day_narrative', {}).get('include_comparisons', True)
        self.auto_generate = config.get('day_narrative', {}).get('auto_generate', True)
        self.generation_time = config.get('day_narrative', {}).get('generation_time', '21:00')
        ...
        self.narrative_templates = {
            'friendly': {
                ...
                'transitions': [
                    "Następnie", "Potem", "W międzyczasie", "Dalej", "Później"
                ],
                'closing': [
            ...
            'professional': {
                ...
                'transitions': [
                    "Następnie zaobserwowano", "W kolejnej fazie", "Kontynuując", "Dalej", "W trakcie"
                ],
                'closing': [
            ...
            'casual': {
                ...
                'transitions': [
                    "A potem", "I wtedy", "No i", "Później", "A następnie"
                ],
                'closing': [
            ...
            'poetic': {
                ...
                'transitions': [
                    "Niczym fale na morzu", "W harmonii z czasem", "Jak nić w tkaninie", "W naturalnym rytmie"
                ],
                'closing': [
        ...
        self.emotion_keywords = {
            'productive': ['completed', 'finished', 'accomplished', 'productive', 'efficient'],
    ...
    async def initialize(self):
        ...
        if not AI_MODULE_AVAILABLE:
            logger.warning("AI module not available - narratives will use templates only")
        
        logger.info("Day Narrative Module initialized")
    
    async def generate_day_narrative(self, date: Optional[str] = None, style: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(server_app, 'day_summary'):
                ...
                if day_summary.get('success'):
                    ...
                    day_data['summary']['total_active_time'] = day_summary['statistics']['total_active_time_hours']
                    day_data['summary']['interactions'] = day_summary['statistics']['total_interactions']
                    day_data['summary']['productivity'] = day_summary['statistics']['productivity_score']
            
            # Get behavior insights
            if hasattr(server_app, 'user_behavior'):
                behavior_insights = await server_app.user_behavior.get_behavior_insights()
            ...
            if hasattr(server_app, 'routines_learner'):
                routine_insights = await server_app.routines_learner.get_routine_insights()
            ...
            if hasattr(server_app, 'db_manager'):
                interactions = await server_app.db_manager.get_user_history('1')  # Main user
                # Filter interactions for the specific date
                date_interactions = [
                    interaction for interaction in interactions
                    if interaction.get('timestamp', '').startswith(date)
                ]
                day_data['interactions'] = date_interactions[-20:]  # Last 20 interactions
    ...
    def _analyze_emotions(self, day_data: Dict) -> List[str]:
        ...
        try:
            ...
            if day_data.get('day_summary'):
                productivity = day_data['day_summary']['statistics'].get('productivity_score', 0)
                if productivity > 0.8:
            ...
            if day_data.get('day_summary'):
                ...
                break_time = day_data['day_summary']['statistics'].get('break_time_hours', 0)
    ...
    def _identify_achievements(self, day_data: Dict) -> List[str]:
        ...
        try:
            ...
            # Long focus sessions
            if day_data.get('day_summary'):
                active_time = day_data['day_summary']['statistics'].get('total_active_time_hours', 0)
                if active_time > 8:
            ...
            if day_data.get('routines_data'):
                ...
                if routine_insights.get('statistics', {}).get('recent_sequences', 0) > 5:
...
"""
    ...
    def _build_narrative_context(self, day_data: Dict, style: str) -> str:
        ...
        if day_data.get('day_summary'):
            ...
            context_parts.append(f"Czas aktywności: {stats.get('total_active_time_hours', 0):.1f}h")
            context_parts.append(f"Interakcje: {stats.get('total_interactions', 0)}")
            context_parts.append(f"Produktywność: {stats.get('productivity_score', 0):.2f}")
        ...
        interactions = day_data.get('interactions', [])
        if interactions:
            interaction_types = [interaction.get('role', 'unknown') for interaction in interactions[-5:]]
            context_parts.append(f"Ostatnie interakcje: {len(interactions)} ({', '.join(set(interaction_types))})")
    ...
    async def _generate_template_narrative(self, day_data: Dict, style: str) -> str:
        ...
        try:
            template = self.narrative_templates.get(style, self.narrative_templates['friendly'])
            ...
            closing = random.choice(template['closing']).format(
                period='dzień' if datetime.fromisoformat(day_data['date']).weekday() < 5 else 'dzień weekendowy'
            )
    ...
    async def compare_days(self, date1: str, date2: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if AI_MODULE_AVAILABLE:
                ...
                comparison = await self._generate_template_comparison(day1_data, day2_data)
...
"""
            ...
            if response_json:
                try:
                    ...
                    return response_json.strip() if isinstance(response_json, str) else ""
    ...
    async def _generate_template_comparison(self, day1_data: Dict, day2_data: Dict) -> str:
        ...
        try:
            ...
            if day1_data.get('day_summary') and day2_data.get('day_summary'):
                prod1 = day1_data['day_summary']['statistics'].get('productivity_score', 0)
                prod2 = day2_data['day_summary']['statistics'].get('productivity_score', 0)
            ...
            # Compare active time
            if day1_data.get('day_summary') and day2_data.get('day_summary'):
                time1 = day1_data['day_summary']['statistics'].get('total_active_time_hours', 0)
                time2 = day2_data['day_summary']['statistics'].get('total_active_time_hours', 0)
                
                time_diff = abs(time1 - time2)
                if time_diff > 1:  # Significant difference
                    longer_day = day1_data['date'] if time1 > time2 else day2_data['date']
            ...
            if not comparison_parts:
                comparison_parts.append("Oba dni były bardzo podobne pod względem aktywności.")
...
async def generate_day_narrative(user_id: str, date: str = None, style: str = None) -> Dict[str, Any]:
    """Wygeneruj narracyjne podsumowanie dnia."""
    try:
        from server_main import server_app
