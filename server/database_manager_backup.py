...
class DatabaseManager:
    ...
    def _init_database(self):
        ...
        with self.get_db_connection() as conn:
            ...
            # Tabela statusu pluginów użytkowników
            conn.execute('''
            ...
            conn.execute('CREATE INDEX IF NOT EXISTS idx_user_plugins_user_id ON user_plugins (user_id)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_user_plugins_plugin_name ON user_plugins (plugin_name)')
    ...
    async def get_user_history(self, user_id: str, limit: int = 50) -> List[Dict]:
        ...
        db_user_id = await self.get_user_id(user_id)
        if not db_user_id:
    ...
    async def save_interaction(self, user_id: str, query: str, response: str):
        ...
        # Get or create user and get their integer ID
        db_user_id = await self.get_or_create_user(user_id)
    ...
    # === ZARZĄDZANIE PLUGINAMI UŻYTKOWNIKÓW ===
      async def update_user_plugin_status(self, user_id: str, plugin_name: str, enabled: bool) -> bool:
        """Aktualizuje status pluginu dla użytkownika."""
        try:
            # Get or create user and get their integer ID
            db_user_id = await self.get_or_create_user(user_id)
            ...
            with self.get_db_connection() as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    INSERT OR REPLACE INTO user_plugins (user_id, plugin_name, enabled, updated_at)
                    VALUES (?, ?, ?, CURRENT_TIMESTAMP)
                ''', (db_user_id, plugin_name, enabled))
            ...
            logger.error(f"Error updating plugin status for user {user_id}: {e}")
            return False
      def get_user_plugin_status(self, user_id: str, plugin_name: str = None) -> Dict[str, bool]:
        """Pobiera status pluginów dla użytkownika."""
        try:
            # This is a sync method, so we need to handle user lookup differently
            # For now, we'll try to get the user ID, and if it doesn't exist, return empty
            with self.get_db_connection() as conn:
                # First get the user's integer ID
                user_cursor = conn.execute("SELECT id FROM users WHERE username = ?", (user_id,))
                ...
                if plugin_name:
                    cursor.execute(
                        "SELECT enabled FROM user_plugins WHERE user_id = ? AND plugin_name = ?",
                        (db_user_id, plugin_name)
                    )
                    row = cursor.fetchone()
                    return {plugin_name: bool(row['enabled']) if row else False}
                else:
                    cursor.execute(
                        "SELECT plugin_name, enabled FROM user_plugins WHERE user_id = ?",
                        (db_user_id,)
                    )
