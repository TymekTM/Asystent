"""
Security Permissions Fixer
Fixes file permissions for production security
"""
import os
import stat
import logging
...
class PermissionsFixer:
    ...
    def fix_config_permissions(self) -> bool:
        ...
        for pattern in config_files:
            for config_file in self.base_path.rglob(pattern):
                ...
                if any(skip in str(config_file) for skip in ['node_modules', '.git', '__pycache__']):
                    continue
                    
                try:
                    # Set permissions to 600 for sensitive configs
                    if any(sensitive in config_file.name.lower() 
                          for sensitive in ['.env', 'auth', 'secret', 'key']):
                        os.chmod(config_file, stat.S_IRUSR | stat.S_IWUSR)
                        logger.info(f"Fixed sensitive config permissions: {config_file}")
                    else:
    ...
    def fix_script_permissions(self) -> bool:
        ...
        for pattern in script_patterns:
            for script_file in self.base_path.rglob(pattern):
                # Skip certain directories
                if any(skip in str(script_file) for skip in ['node_modules', '.git', '__pycache__']):
                    continue
    ...
    def audit_permissions(self) -> Dict[str, List[str]]:
        ...
        for db_file in self.base_path.rglob("*.db"):
            ...
            if mode != "600":
                issues["critical"].append(f"Database {db_file} has permissions {mode}, should be 600")
        
        # Check .env files
        for env_file in self.base_path.rglob(".env*"):
            mode = oct(os.stat(env_file).st_mode)[-3:]
            if mode != "600":
                issues["critical"].append(f"Environment file {env_file} has permissions {mode}, should be 600")
            ...
            for config_file in self.base_path.rglob(pattern):
                if config_file.is_file():
                    ...
                    if mode not in ["600", "640"]:
                        issues["warning"].append(f"Sensitive config {config_file} has permissions {mode}")
