...
def debug_function_calling():
    ...
    try:
        ...
        print(
            f"Plugin manager function_registry length: {len(plugin_manager.function_registry)}"
        )
        ...
        for module_name, module in server_modules:
            ...
            try:
                if hasattr(module, "get_functions"):
                    ...
                    print("  - Has get_functions: NO")
                    print(
                        f"  - Available attributes: {[attr for attr in dir(module) if not attr.startswith('_')][:5]}..."
                    )
