...
class FunctionCallingSystem:
    ...
    def __init__(self, modules: Dict[str, Any]):
        ...
        # Add system functions first
        system_functions = self._create_system_functions()
    ...
    def _create_system_functions(self) -> List[Dict[str, Any]]:
        """Create system control functions like listen_after_tts."""
        system_functions = []
        
        # Listen after TTS function
        listen_function = {
            "type": "function",
            "function": {
                "name": "system_listen_after_tts",
                "description": "Enable listening for another voice command after TTS response. Use when user expects to continue the conversation or give another command.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "enabled": {
                            "type": "boolean",
                            "description": "Whether to listen for another command after speaking the response"
                        }
                    },
                    "required": ["enabled"]
                }
        ...
        system_functions.append(listen_function)
        
        # Register handler for the listen function
        self.function_handlers["system_listen_after_tts"] = self._handle_listen_after_tts
        
        return system_functions
    
    def _handle_listen_after_tts(self, enabled: bool = True, **kwargs) -> str:
        """Handle the listen after TTS system function."""
        # This will be processed by the assistant to set the listen flag
        return f"SYSTEM_LISTEN_AFTER_TTS:{enabled}"
