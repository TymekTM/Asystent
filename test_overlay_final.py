...
def check_server_cpu():
    ...
    try:
        result = subprocess.run([
            "docker", "stats", "gaja-assistant-server", 
            "--format", "{{.CPUPerc}}", "--no-stream"
...
def run_old_overlay_test():
    ...
    process = subprocess.Popen([str(current_overlay_path)], 
    ...
    print(f"📊 Wyniki:")
    print(f"   - CPU serwera: {cpu_usage:.2f}%" if cpu_usage else "   - CPU serwera: N/A")
...
async def run_client_with_overlay():
    ...
    ], cwd="f:/Asystent", stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
async def main():
    ...
    print(f"   Baseline CPU: {baseline_cpu:.2f}%" if baseline_cpu else "   Baseline CPU: N/A")
    ...
    if websocket_result:
        ...
        print(f"   - CPU serwera: {websocket_result['cpu_usage']:.2f}%" if websocket_result['cpu_usage'] else "   - CPU serwera: N/A")
    ...
    if websocket_result and websocket_result['cpu_usage']:
        ...
        print(f"   - Redukcja CPU: -{cpu_improvement:.2f}% ({cpu_improvement/2.8*100:.1f}% poprawa)")
